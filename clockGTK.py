...
TIMER:          t (initiate counter) / s (stop counter) - mm:ss
MOVE:           Mouse+Button1
...
class MyWindow(Gtk.Window):
    ...
    def __init__(self):
        ...
        self.connect("button-press-event", self.on_click_in)
        self.connect("motion-notify-event", self.on_motion)
        self.connect("button-release-event", self.on_click_out)
        self.connect("key-press-event", self.on_key_press)
        self.connect("delete-event", lambda y, z: Gtk.main_quit())

        self.tran_setup()
        self.set_title("Clock by alef")
        self.set_icon_from_file(utils.get_resource_path("resources/clock.ico"))
        ...
        self.alarm_image.set_from_file(utils.get_resource_path("resources/Alarm_set.png"))
        self.alarm_not_set_image = Gtk.Image()
        self.alarm_not_set_image.set_from_file(utils.get_resource_path("resources/Alarm_not_set.png"))
        self.image = None
        ...
        self.hour_alarm.set_tooltip_text("Enter hours (HH)")
        ...
        self.min_alarm.set_tooltip_text("Enter minutes (MM)")
        ...
        self.min_entry.set_tooltip_text("Enter minutes (MM)")
        ...
        self.sec_entry.set_tooltip_text("Enter seconds (SS)")
    ...
    def get_alarm_values(self):
        ...
        self.hour_alarm.set_text(time.strftime("%H"))
        self.time_label.pack_start(self.hour_alarm, expand=True, fill=True, padding=0)
        self.time_label.pack_start(self.values_label, expand=True, fill=True, padding=0)
        self.min_alarm.set_text(time.strftime("%M"))
    ...
    def beep(self):
        ...
        t = threading.Thread(target=utils.notify, args=(message, self.beep_sound, utils.get_resource_path("resources/clock.ico")))
        t.start()
    ...
    def on_click_in(self, widget, event):
        self.mouse_X_pos = event.x
        self.mouse_Y_pos = event.y

    def on_motion(self, widget, event):
        if self.mouse_X_pos is not None:
            self.move(self.get_position().root_x - (self.mouse_X_pos - event.x), self.get_position().root_y - (self.mouse_Y_pos - event.y))

    def on_click_out(self, widget, event):
        self.mouse_X_pos = None
        self.mouse_Y_pos = None
    ...
    def on_key_press(self, widget, event):
        ...
        elif event.keyval == Gdk.keyval_from_name("t"):         # t, T --> Counter mode
            if self.clock_mode:
